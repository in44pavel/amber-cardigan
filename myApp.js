...
var Person = mongoose.model("Person", schemaPerson); 
...
Person.findOne(food.name,function (err,data) {
    if (err) 
        return done(err);
    return done(null, data);
    });
...
var findPersonById = function(personId, done) {
  Person.findById(personId._id,function (err,data) {
    if (err) 
        return done(err);
    return done(null, data);
    });
...
var findEditThenSave = function(personId, done) {
  ...
  Person.findById(personId._id,function (err,data) {
    ...
    data.favoriteFoods.push(foodToAdd);//returns 2
